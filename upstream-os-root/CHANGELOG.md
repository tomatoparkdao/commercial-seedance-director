...
- Repaired the generic copyable prompts in the active reference package. Repository-authored tag examples now use canonical `@Video1` and `@Image1`; user- or interface-supplied tags such as `[Video 1]` and `@Image 1` remain byte-preserved. Continuation examples start from an accepted take's observed end state instead of a planned ending, and the weakest showcase prompts now carry brief-specific behavior, props, timing, sound, and endpoints. Focused regressions inspect the inline prompts users actually copy.
...
- Restored first- and last-frame extraction on current FFmpeg builds without dropping older-build compatibility. The helper probes the selected binary once, uses `-fps_mode passthrough` when available, and otherwise falls back to `-vsync 0`; the same real-frame path was verified with FFmpeg 8.1.1 and 9.0.
- Bound first publication of a new evaluator ledger to retained file and directory descriptors or handles. Linux publishes an unnamed `O_TMPFILE` with atomic no-replace descriptor linking; Windows publishes the retained zero-share file handle relative to the retained target-directory handle. Rollback is armed before the native call, late destination claimants and namespace substitutes are preserved, exact owned objects are invalidated or removed, and hosts without the required safe primitive fail closed.
- Repaired Windows masthead-build runner trust for the supported CPython 3.11–3.13 range. Installed 3.11/3.12 builds trust their exact packaged `python[_d].exe`; installed 3.13 builds trust `venvlauncher[t][_d].exe`; source builds select the corresponding launcher beside the base interpreter. Missing, unsupported, and ABI/debug-mismatched variants fail closed, and CI exercises Python 3.11, 3.12, and 3.13.

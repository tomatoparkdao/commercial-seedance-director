...
const workflow_1 = require("./scripts/workflow");
const tools = [
...
async function handle(req) {
    if (req.method === 'initialize')
        return response(req.id, { protocolVersion: '2024-11-05', capabilities: { tools: {} }, serverInfo: { name: 'commercial-seedance-director', version: '1.1.0' } });
    if (req.method === 'tools/list')
        return response(req.id, { tools });
    if (req.method !== 'tools/call')
        return response(req.id, { error: { code: -32601, message: `Unknown method: ${req.method}` } });
    const p = req.params || {}, name = String(p.name || ''), a = (p.arguments || {});
    try {
        if (name === 'create_brief_card')
            return response(req.id, { content: [{ type: 'text', text: JSON.stringify((0, workflow_1.createBriefCard)(String(a.user_input || ''))) }] });
        if (name === 'confirm_direction')
            return response(req.id, { content: [{ type: 'text', text: JSON.stringify((0, workflow_1.confirmDirection)(a)) }] });
        if (name === 'submit_render')
            return response(req.id, { content: [{ type: 'text', text: JSON.stringify(await (0, workflow_1.submitRender)(a)) }] });
        if (name === 'poll_render_progress')
            return response(req.id, { content: [{ type: 'text', text: JSON.stringify(await (0, workflow_1.pollRenderProgress)(a.task_ids)) }] });
        return response(req.id, { isError: true, content: [{ type: 'text', text: `Unknown tool: ${name}` }] });
    }
    catch (error) {
        return response(req.id, { isError: true, content: [{ type: 'text', text: error instanceof Error ? error.message : String(error) }] });
    }
}
let buffer = '';
...
process.stdin.on('data', chunk => { buffer += chunk; const lines = buffer.split(/\r?\n/); buffer = lines.pop() || ''; for (const line of lines)
    if (line.trim())
        void handle(JSON.parse(line)).then(output => process.stdout.write(output + '\n')); });

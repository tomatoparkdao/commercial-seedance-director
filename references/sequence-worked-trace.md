...
The idea ("my character lands and is driven away") is bigger than one generation, so the Sequence Gate classifies it `sequence_project` and each performed clip receives a `narrative` Director's Read lane. Before Clip 01 exists, the plan fixes: the story promise and final outcome, beats grouped into **scenes**, and per clip a `narrative_job`, a `felt_intent`, the complete canonical internal `authoring_state`, and a completed endpoint. The airport fixture's creased paper airline tag is explicitly marked `authored_choice`, never misrepresented as visible in `@Image 1`. Its thumb-catch and fold replace the refused shoulder-drop/backward-glance defaults and survive even if a different agent writes Clip 02.
...
The contract (`clip-01-contract.json`) carries the job ("Exit terminal and approach the open rear car door"), the felt intent ("the quiet relief of arrival"), the complete planned authoring record, the reserved beats (entering the car, departure), and the locks. The compiler (see [prompt-compiler](prompt-compiler.md)) emits one natural-language prompt (`clip-01-prompt.md`) - T2V, since nothing exists yet. The visible carriers ship; canonical labels and internal JSON do not.
...
Verdict: `accept_with_deviation`. Reconciliation (see [sequence-project-state](sequence-project-state.md)) updates project canon: the deviation is recorded, the observed end state and revised authored turn live in project state, and Clip 02's current contract opens **two steps out, mid-stride**. Clip 01's status becomes `accepted_with_deviation`, but its historical contract and prompt keep the planned endpoint; they are not rewritten to imitate observed truth. Rejected takes, by contrast, change nothing: rejected footage never becomes canon or a parent source.

## 3 - Continue from what actually happened

Clip 02 (`clip-02-continuation-contract.json`) is a `seamless_continuation` inside `scene_01`: parent `clip_01`, `extension_depth: 1`, sourced from the accepted take attached as `[Video 1]`. Its `value_before` exactly matches Clip 01's revised `value_after`; this is the dramatic handoff beside the physical one. The **source-carries-state rule** applies: the video reference carries the opening state, so the prompt text carries only the delta - finish the approach, enter, close the door, the exact filmable authoring carriers, the felt-intent carrier (the door thump cutting curb noise), and the reserved-beat exclusion (no departure yet). Prose that re-describes what `[Video 1]` already shows would be budget spent turning disagreement into drift.
...
Sessions end; canon must not. The Project State Capsule (template in [sequence-project-state](sequence-project-state.md)) carries the compact project in under ~50 lines: scene map (completed scenes compressed to one line each), current scene, current actual state, open motion, completed beats, next clip job and intent, the expanded internal authoring state, its visible carriers, locks, reserved beats, extension depth, and uncertainties. A new session pastes the capsule and continues from the recorded physical and dramatic state - no hidden memory is assumed, and the machine truth (`project-state.json`, per the State Lifecycle) regenerates the capsule whenever `state_revision` bumps.
